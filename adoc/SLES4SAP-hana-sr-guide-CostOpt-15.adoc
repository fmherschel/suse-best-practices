...
+
.{hana} System Replication Scale-Up in the Cluster - perfomance optimized
...
+
.{hana} System Replication Scale-Up in the Cluster - perfomance optimized chain
...
+
.{hana} System Replication Scale-Up in the Cluster - perfomance optimized multi target
...
The automated shutdown of the non-replicated {SAPHANA} database (e.g. QAS) is achieved by cluster
rules. More precisely, it is an anti-colocation of {SAPHANA} promoted vs. {SAPHANA} non-replicated. This means
...
section is available for exaample for {HANA} 2.0 SPS05 at https://help.sap.com/viewer/6b94445c94ae495c83a19646e7c3fd56/2.0.05/en-US/5447545b91a04cf8a0d6133a026f2be5.html.
...
* Name resolution of the cluster nodes and the virtual IP address must be done locally on all cluster nodes. If that is not possible, additional measures are needed to ensure reliable resolution of hostnames at any time.
...
After that happend, the initial state has to be restored by the administrative
...
* {hana} versions:
** {hana} 1.0 is supported since SPS09 (095) for all mentioned setups. {hana} 1.0 does not support the HA/DR provider hook method srConnectionChanegd() with multi-target aware parameters. These parameters are needed for the HA/DR hook in the package.
...
Also, take a look at the {HANA} product availability matrix, which can for exmaple be found at
...
// DONE PRIO2: install supportutil-plugin-ha-sap, see https://github.com/SUSE/suse-best-practices/issues/122#issuecomment-753994628

Optionally the packages supportutil-plugin-ha-sap and ClusterTools2 can be installed. The first helps collecting data for support requests, the second simplifies common administrative tasks.
...
// TODO PRIO2: Add sizing refrence here
[subs="specialchars,attributes,quotes"]
----
[memorymanager]
...
// TODO PRIO2: CostOpt15 - Check the following procedure - still correct steps and order?
...
package. The other is named {haDrCostOptMem} and needs to be adopted to your database according to
...
directory, such as /hana/shared/srHook/. The hook must be available on all {SAPHANA} cluster nodes.
...
In our example we would have four separate lines with Cmnd_Alias entries, one line for the {refsidadm} user and one or more lines for comments.
...
It is recommended to use symlink to have those Python libraries available. Using symlink, the files are always up to date if you update {hana}.
[subs="specialchars,attributes"]
----
# cd /hana/shared/srHook
...
// TODO PRIO2: Describe the hook test (e.g. restart secondary) to trigger the write of cluster attributes
...
_Adding the Second Node_ of the {dochaquickstart} for {suselinux} {sleha} {pn15} {psp15} at {haquickstart15}.
...
for {suselinux} {sleha} {pn15}, and the TIDs 7016880 and 7008216.
...
Administration Guide for SUSE Linux Enterprise High Availability Extension 15.
...
For an advanced SBD setup, refer to the {suselinux} {sleha} product documentation (for example, visit
https://documentation.suse.com/sle-ha/15-SP2/html/SLE-HA-all/cha-ha-storage-protect.html#pro-ha-storage-protect-fencing).
...
Availability Extension at {haAdminGuide15}.
...
// TODO PRIO2: CostOpt15 - Use variables as placeholders
...
//             make it nore easy to review srHook and RA attributes (like SOK or SFAIL)
...
on the exit state of the preceeding tests.
...
Mon Nov 11 21:15:43 2019; Hosts/{sapnode2}/roles=4:P:master1:master:worker:master
----
...
The cluster did jump in and initiated a takeover. At 21:15 (CET) the former secondary
...
To receive updates for the operating system or the {suselinux} {sleha},
...
* You might want to revert the {HANA} roles back soon, to get the non-replicated database also up and running again.
...
Current online documentation for SUSE Linux Enterprise Server for SAP Applications 15::
 https://documentation.suse.com/sles-sap/15-SP2/

Current online documentation for {sleha} 15::
 https://documentation.suse.com/sle-ha/15-SP2/

System Analysis and Tuning Guide for {sls} 15::
 https://documentation.suse.com/sles/15-SP2/html/SLES-all/book-sle-tuning.html

Storage Administration Guide for {sls} 15::
 https://documentation.suse.com/sles/15-SP2/single-html/SLES-storage/#stor-admin
...
// TODO PRIO4: URL for SUSE Manager and RMT
...
// TODO PRIO2: 2369981 - Required configuration steps for authentication with HANA System Replication
// TODO PRIO2: 3007062 -
...
and instance number {sapino}. The standalone database has SID QAS and instance number 20.The virtual IP address in the example is
{sapip1srv1}
...
// TODO PRIO2: 15 SP1: property $id="cib-bootstrap-options" \ ?
// TODO PRIO2: 15 SP1: rsc_defaults $id="rsc-options" \ ?
// TODO PRIO2: 15 SP1: op_defaults $id="op-options" \ ?
