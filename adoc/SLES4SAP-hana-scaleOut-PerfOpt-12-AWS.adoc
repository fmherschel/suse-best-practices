...
DONE PRIO1: make sure "no-quorum-policy = suicide"
...
https://aws.amazon.com/premiumsupport/knowledge-center/linux-static-hostname-suse/.
...
Public key authentication provides passwordless SSH access to servers.

This setup is based on only one SSH key pair which enables SSH access to all cluster nodes.
...
Collect the public host keys from all other node. We have used the _ssh-keyscan_ command.
...
https://docs.aws.amazon.com/quickstart/latest/sap-hana/planning.html.
The table on this Web site lists the minimum required number of EBS volumes, volume size
