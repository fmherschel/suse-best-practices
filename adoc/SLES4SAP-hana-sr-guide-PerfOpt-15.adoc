...
if the {saphana} system replication brakes. In addition to the {saphana} hook status
...
available at:
https://documentation.suse.com/sles-sap/15-SP1/single-html/SLES4SAP-guide/#cha-s4s-cluster
...
// DONE PRIO2: install ClusterTools2
...
// from the file /etc/sysconfig/SBD (2020-02-05)
Defines the timeout, in seconds, the watchdog will wait before panicking the node if no-one tickles it.

This parameter must match your storage environemt. If you expect your storage could hang for 30s, then
...
The parameter must also be aligned with the pacameker cluster property stonith-watchdog-timeout.
The property stonith-watchdog-timeout must be set >= the value of SBD_WATCHDOG_TIMEOUT. 

// from the product documentation - copied 2020-02-05
If you set `stonith-watchdog-timeout` to a negative value, Pacemaker will automatically calculate this timeout and set it to twice the value of `SBD_WATCHDOG_TIMEOUT` starting with SUSE Linux Enterprise High Availability Extension 15. 
...
device names. As an example the `SBD_WATCHDOG_TIMEOUT` is set to 15s to be less
aggressive than the typical 5s.
...
Last change: Wed Feb  5 15:04:42 2020 by hacluster via crmd on {sapnode2}
2 nodes configured
...
Last change: Thu Feb  6 12:19:43 2020 by root via crm_attribute on {sapnode1}
