...
- HANA host auto-fail-over for all above

From the infrastructure perspective the following variants are covered:

- 3-site cluster with disk-based and disk-less SBD fencing
- 1-site cluster with disk-based and disk-less SBD fencing
...
auto-fail-over. That means there are standby nodes which are managed by {saphana}.
...
setup in a series of blog articles on {#towardsZeroDowntime}, e.g. 
...
from SAPHanaSR-ScaleOut version 0.160, please consult manual page
SAPHanaSR-manageAttr(8) and blog article
...
To automate the fail-over, the High Availability Extension built into
...
perform a host auto-fail-over for the worker functionality.
Please find more details on concept and implementation in manual page
...
Please find configuration details in manual page ocf_suse_SAPHanaController(7).
...
* The cluster must include a valid STONITH method.
  SBD disk-based or disk-less is recommended STONITH method.
...
* The {SAPHANA} scale-out system must have only *one* fail-over group.
* There is only one additional {saphana} system replication connected from
  outside the Linux cluster. Thus two sites are managed by the Linux cluster,
  one site outside is recognized.
* The setup implements the performance optimized scenario but not the cost
  optimized scenario.
* Only one {SAPHANA} SID is installed. Thus the cost-optimized and MCOS
  scenarios are currently not supported.
* The _{sapHostAgent}_ must be running. _{sapHostAgent}_ is needed to translate
  ...
  of {saphana}.
* The replication mode should be either 'sync' or 'syncmem'. 'async' is not
  supported.
...
Please find more details in the REQUIREMENTS section of manual pages
...
the {saphana} storage API can be used. Please refer to the {saphana} TDI
documentation for supported storages and follow the respective storage vendor's 
...
=== Configure {sles4sap} to Run {sapHana}
...
// TODO PRIO3: update below SAP note content
////
The SAP note 2684254 covers the following modifications:
...
After collecting all host keys store them in a file named `authorized_keys`. Push the entire
directory `/root/.ssh/` from the first node to all further cluster members.
...
=== Implement the HA/DR provider hook {haDrMultiTargetPy}
...
The HA/DR hook script is shipped with the SAPHanaSR-scaleOut package in directory /usr/share/SAPHanaSR-ScaleOut/.
The hook script must be available on all cluster nodes, including the majority
maker. Please find more details in manual page SAPHanaSrMultiTarget.py(7).
...
The user {refsidadm} must be able to set the cluster attribute
hana_{refsidLC}_site_srHook_* .
...
==== Initial Cluster Setup Using _ha-cluster-init_
...
Please find more details and examples in manual page SAPHanaSR-ScaleOut_basic_cluster(7).
...
Please find more details in manual page ocf_suse_SAPHanaTopology(7).
...
This is to give an administrator the chance to react on a fail-over. If the complete node
...
The cluster is idle, no actions are pending, no migration constraints left over, no fail-counts left over.
...
storage location. Please find more details and examples in manual page
...
// DONE PRIO1: replace global srHook by site-specific 
[subs="specialchars,attributes"]
----
...
TID Configuring Persistent Memory Devices (PMEM) results in booting to the recovery shell::
 https://www.suse.com/support/kb/doc/?id=000019517

TID Slow boot boot initialization on machines with Intel Optane DC Memory causing auto-mount to fail:: 
 https://www.suse.com/support/kb/doc/?id=000019462
